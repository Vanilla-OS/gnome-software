...
G_DEFINE_TYPE(GsTariffEditor, gs_tariff_editor, GTK_TYPE_GRID)

G_DEFINE_QUARK(GsTariffEditorError, gs_tariff_editor_error)

enum { TARIFF_CHANGED, LAST_SIGNAL };
...
static void on_time_changed_cb(GsTariffEditor *self);
...
static gint get_am_hour(GDateTime *dt) {
...
static void update_seconds_from_adjustments(GsTariffEditor *self) {
   ...
   * need to add +1 day to the end date. Otherwise, e.g, [13:10, 19:15), the
   * period is contained in a single day, and we don't need to do anything.
   */
...
static gboolean setup_tariff(GsTariffEditor *self, MwtTariff *tariff,
                             GError **error) {
  ...
  if (periods->len != 2) {
    g_set_error(error, GS_TARIFF_EDITOR_ERROR,
  ...
  if (mwt_period_get_repeat_type(base_period) != MWT_PERIOD_REPEAT_NONE ||
      ...
      g_date_time_to_unix(mwt_period_get_end(base_period)) !=
          g_date_time_to_unix(maximum_date)) {
    g_set_error(error, GS_TARIFF_EDITOR_ERROR,
                GS_TARIFF_EDITOR_ERROR_WRONG_FORMAT, "Base tariff is wrong");
    return FALSE;
  }
  ...
  g_signal_handlers_block_by_func(self->adjustment_from_hours,
                                  on_time_changed_cb, self);
  g_signal_handlers_block_by_func(self->adjustment_from_minutes,
                                  on_time_changed_cb, self);
  g_signal_handlers_block_by_func(self->adjustment_to_hours, on_time_changed_cb,
                                  self);
  g_signal_handlers_block_by_func(self->adjustment_to_minutes,
                                  on_time_changed_cb, self);
  ...
  if (self->clock_format == G_DESKTOP_CLOCK_FORMAT_24H) {
    gtk_adjustment_set_value(self->adjustment_from_hours,
                             g_date_time_get_hour(period_start));
    gtk_adjustment_set_value(self->adjustment_from_minutes,
                             g_date_time_get_minute(period_start));
    gtk_adjustment_set_value(self->adjustment_to_hours,
                             g_date_time_get_hour(period_end));
    gtk_adjustment_set_value(self->adjustment_to_minutes,
                             g_date_time_get_minute(period_end));
  } else {
    ...
    gtk_stack_set_visible_child(GTK_STACK(self->stack_from),
                                self->time_period_from == AM
                                    ? GTK_WIDGET(self->button_from_am)
                                    : GTK_WIDGET(self->button_from_pm));
    gtk_stack_set_visible_child(GTK_STACK(self->stack_to),
                                self->time_period_to == AM
                                    ? GTK_WIDGET(self->button_to_am)
                                    : GTK_WIDGET(self->button_to_pm));

    gtk_adjustment_set_value(self->adjustment_from_hours,
                             get_am_hour(period_start));
    gtk_adjustment_set_value(self->adjustment_to_hours,
                             get_am_hour(period_end));
    gtk_adjustment_set_value(self->adjustment_from_minutes,
                             g_date_time_get_minute(period_start));
    gtk_adjustment_set_value(self->adjustment_to_minutes,
                             g_date_time_get_minute(period_end));
  ...
  g_signal_handlers_unblock_by_func(self->adjustment_from_hours,
                                    on_time_changed_cb, self);
  g_signal_handlers_unblock_by_func(self->adjustment_from_minutes,
                                    on_time_changed_cb, self);
  g_signal_handlers_unblock_by_func(self->adjustment_to_hours,
                                    on_time_changed_cb, self);
  g_signal_handlers_unblock_by_func(self->adjustment_to_minutes,
                                    on_time_changed_cb, self);

  return TRUE;
}

static void update_tariff(GsTariffEditor *self, gboolean should_notify) {
  ...
  forbidden_period =
      mwt_period_new(forbidden_start, forbidden_end, MWT_PERIOD_REPEAT_NONE, 0,
                     "capacity-limit", 0, NULL);
  ...
  allowed_period =
      mwt_period_new(allowed_start, allowed_end, MWT_PERIOD_REPEAT_DAY, 1,
                     "capacity-limit", G_MAXUINT64, NULL);
  ...
  self->tariff_as_variant =
      mwt_tariff_builder_get_tariff_as_variant(tariff_builder);
...
static void update_adjustments(GsTariffEditor *self) {
  ...
  if (self->clock_format == G_DESKTOP_CLOCK_FORMAT_24H) {
    ...
    self->time_period_from =
        gtk_adjustment_get_value(self->adjustment_from_hours) < 12 ? AM : PM;
  ...
  if (self->clock_format == G_DESKTOP_CLOCK_FORMAT_24H) {
    gtk_stack_set_visible_child(GTK_STACK(self->stack_to),
                                GTK_WIDGET(self->blank_to));

    gtk_adjustment_set_lower(self->adjustment_to_hours, 0);
    gtk_adjustment_set_upper(self->adjustment_to_hours, 23);

    gtk_adjustment_set_value(self->adjustment_to_hours,
                             g_date_time_get_hour(end));
  } else {
    self->time_period_to =
        gtk_adjustment_get_value(self->adjustment_to_hours) < 12 ? AM : PM;
...
static void on_clock_settings_changed_cb(GSettings *settings_display,
                                         gchar *key, GsTariffEditor *self) {
  self->clock_format = g_settings_get_enum(settings_display, CLOCK_FORMAT_KEY);

  update_adjustments(self);
}

static gboolean on_hours_output_cb(GtkSpinButton *spin, GsTariffEditor *self) {
...
static gboolean on_minutes_output_cb(GtkSpinButton *spin,
                                     GsTariffEditor *self) {
  GtkAdjustment *adjustment;
  g_autofree gchar *text = NULL;
...
static void on_time_period_from_clicked_cb(GtkButton *button,
                                           GsTariffEditor *self) {
  if (self->time_period_from == AM) {
...
static void on_time_period_to_clicked_cb(GtkButton *button,
                                         GsTariffEditor *self) {
  if (self->time_period_to == AM) {
...
static void gs_tariff_editor_finalize(GObject *object) {
  GsTariffEditor *self = (GsTariffEditor *)object;

  g_clear_object(&self->settings_clock);
  g_clear_pointer(&self->tariff_as_variant, g_variant_unref);

  G_OBJECT_CLASS(gs_tariff_editor_parent_class)->finalize(object);
}

static void gs_tariff_editor_class_init(GsTariffEditorClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS(klass);

  object_class->finalize = gs_tariff_editor_finalize;

  signals[TARIFF_CHANGED] =
      g_signal_new("tariff-changed", GS_TYPE_TARIFF_EDITOR, G_SIGNAL_RUN_FIRST,
                   0, NULL, NULL, NULL, G_TYPE_NONE, 0);

  gtk_widget_class_set_template_from_resource(
      widget_class, "/org/gnome/Software/gs-tariff-editor.ui");

  gtk_widget_class_bind_template_child(widget_class, GsTariffEditor,
  ...
  gtk_widget_class_bind_template_child(widget_class, GsTariffEditor,
                                       stack_from);
  gtk_widget_class_bind_template_child(widget_class, GsTariffEditor, stack_to);
  gtk_widget_class_bind_template_child(widget_class, GsTariffEditor,
                                       button_from_am);
  gtk_widget_class_bind_template_child(widget_class, GsTariffEditor,
                                       button_from_pm);
  gtk_widget_class_bind_template_child(widget_class, GsTariffEditor,
                                       spinbutton_from_hours);
  gtk_widget_class_bind_template_child(widget_class, GsTariffEditor,
                                       spinbutton_from_minutes);
  gtk_widget_class_bind_template_child(widget_class, GsTariffEditor,
                                       blank_from);
  gtk_widget_class_bind_template_child(widget_class, GsTariffEditor,
                                       button_to_am);
  ...
  gtk_widget_class_bind_template_child(widget_class, GsTariffEditor,
                                       spinbutton_to_hours);
  gtk_widget_class_bind_template_child(widget_class, GsTariffEditor,
                                       spinbutton_to_minutes);
  gtk_widget_class_bind_template_child(widget_class, GsTariffEditor, blank_to);
  ...
  gtk_widget_class_bind_template_callback(widget_class,
                                          on_time_period_from_clicked_cb);
  gtk_widget_class_bind_template_callback(widget_class,
                                          on_time_period_to_clicked_cb);
}

static void gs_tariff_editor_init(GsTariffEditor *self) {
  ...
  self->clock_format =
      g_settings_get_enum(self->settings_clock, CLOCK_FORMAT_KEY);

  update_adjustments(self);

  g_signal_connect(self->settings_clock, "changed::" CLOCK_FORMAT_KEY,
                   G_CALLBACK(on_clock_settings_changed_cb), self);

  update_tariff(self, FALSE);
}

/**
 * gs_tariff_editor_get_tariff_as_variant:
 * @self: a #GsTariffEditor
 *
...
GVariant *gs_tariff_editor_get_tariff_as_variant(GsTariffEditor *self) {
  g_return_val_if_fail(GS_IS_TARIFF_EDITOR(self), NULL);

  return self->tariff_as_variant;
}

void gs_tariff_editor_load_tariff(GsTariffEditor *self, MwtTariff *tariff,
                                  GError **error) {
  g_return_if_fail(GS_IS_TARIFF_EDITOR(self));
